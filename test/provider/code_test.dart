...
void main() {
  ...
  bool isBufferDiffInThreshold(DateTime now, DateTime? bufferedAt, int thresholdMilliseconds, {bool isLessThan = true}) {
    if (bufferedAt != null) {
      int diffMs = now.difference(bufferedAt).inMilliseconds;
      print(diffMs);
      return isLessThan? diffMs < thresholdMilliseconds: diffMs > thresholdMilliseconds;
    }
    return false;
  }
    ...
    test('source file modify and sync', () async {
      ...
      expect(container.read(sourceFileProvider(filePath: testFilePath)).value,
          isA<CodeText>()
              ...
              .having((code) => code.fullText.length, 'full code', counterProviderMaterialApp.length)
              .having((code) => code.language, 'language', testFileExt)
      );

      // Fast forward clock by 100ms
      when(mockClock.now()).thenAnswer((_) => now.add(const Duration(milliseconds: 100)));

      // Set up the mock file service to return the modified code
      container.read(sourceFileProvider(filePath: testFilePath).notifier)
      ...
      expect(container.read(sourceFileProvider(filePath: testFilePath)).value,
          isA<CodeText>()
              .having((code) => code.filePath, 'file path', testFilePath)
              .having((code) => code.fullText.length, 'full code', counterProviderMaterialApp2.length)
              .having((code) => code.language, 'language', testFileExt)
      ...
      expect(container.read(sourceFileProvider(filePath: testFilePath)).value,
          isA<CodeText>()
              ...
              .having((code) => code.fullText.length, 'full code', counterProviderMaterialApp.length)
              .having((code) => code.language, 'language', testFileExt)
              .having((code) => isBufferDiffInThreshold(now, code.bufferedAt, 300), 'buffer diff', isTrue)
      );

      // Fast forward clock by 500ms
      when(mockClock.now()).thenAnswer((_) => now.add(const Duration(milliseconds: 500)));
      ...
      container.read(sourceFileProvider(filePath: testFilePath).notifier).syncCode();
